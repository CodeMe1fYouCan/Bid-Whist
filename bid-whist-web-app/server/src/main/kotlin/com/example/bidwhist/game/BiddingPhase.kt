...
suspend fun handleBid(room: RoomState, handId: String, bidAmount: Any, objectMapper: ObjectMapper) {
        ...
        val bidEntry = mutableMapOf<String, Any>("handId" to handId, "handIndex" to bidderIndex)

        if (bidAmount == "pass") {
                if (bidderIndex == dealerIndex && passedPlayers.size == 3 && highestBid == 0) {
                        println(
                                "🎯 Dealer cannot pass when everyone else has passed - forcing bid of 1"
                        )
                        bidEntry["amount"] = 1
                        ...
                        val errorMessage =
                                objectMapper.writeValueAsString(
                                        mapOf(
                                                "type" to "BID_ERROR",
                                                "message" to
                                                        "You cannot pass - you must make a bid!"
                                        )
                                )
                        room.connections[handAssignments[bidderIndex]["playerId"]]?.send(
                ...
                val minBid =
                        if (highestBid == 0) 1 else if (isDealer) highestBid else highestBid + 1
