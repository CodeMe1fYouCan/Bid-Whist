...
suspend fun handlePlayerJoined(
    ...
    if (existingPlayer == null) {
        ...
        // This fixes the issue where backend has stale state (e.g. handCount=3) but frontend reset
        // to 1
...
suspend fun handleToggleReady(
    ...
    println("DEBUG: handleToggleReady called for player $playerId with isReady=$isReady")
    ...
    println("DEBUG: Player before update: ${player.name}, isReady=${player.isReady}")
    player.isReady = isReady
    println("DEBUG: Player after update: ${player.name}, isReady=${player.isReady}")
    println("Player ${player.name} ready state: $isReady")

    // Verify room list
    val playerInList = room.players.find { it.id == playerId }
    println("DEBUG: Player in room list: ${playerInList?.name}, isReady=${playerInList?.isReady}")
